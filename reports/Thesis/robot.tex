...
This angle is measured using a contactless sensor (\ref{item:sensor}) that can measure the on-axis rotation of a small magnet (\ref{item:magnet}) attached to the actuation arm. \todo{discuss mechaical limit}  % TODO: mechanical limit
...
The sensor measures this state and sends the values to an Arduino Nano (\ref{item:arduino}) board, which runs a PID controller that returns the control action, i.e., an angular position for the servo.
...
Upon startup, the Arduino initializes a connection with the external computer and directs the servo to move to its reference position (datum).
The Arduino allows some time for the servo to reach its datum and then reads a first measurement of the angle sensor, setting this as the reference $\theta$.
...
Based on these measurements, the PID controller computes the value to be written to the servo.
...
The target adaptation and supervised learning is implemented exclusively on the external computer through the established connection with the Arduino.
...
For example, the sole task of the \texttt{send\_value} thread is to read the value of a global variable representing the target state and send this to the Arduino.
...
\begin{table}
    ...
    \begin{tabular}{lll}
        ...
        \midrule
        Arduino                & Robot               & $x, \, x^{*}$           \\
        \texttt{send\_value}   & $x^{*}_d$           & Arduino                 \\
        Arduino                & PC                  & $x_d$                   \\
        \texttt{PID}           & $x, \, x_d$         & $u$                     \\
        Arduino                & $u$                 & Robot                   \\
        \texttt{listen}        & Arduino             & $x^{*}$, buffer         \\
        \texttt{change\_value} & $x^{*}, \, x^{*}_d$ & $\hat{x}_d, \, x^{*}_d$ \\
        \texttt{update}        & buffer              & adapter                 \\
        \bottomrule
