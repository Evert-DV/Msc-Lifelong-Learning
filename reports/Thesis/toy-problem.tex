...
The system state $\mathbf{x}$ consists of the position $x$ and velocity $\dot{x}$.
...
Each timestep, the adapter calculates the required $\Delta$ to be added to the intended target \emph{position}, and the controller computes the control action based on the current position and the adapted target position.
The system's response to this control input is simulated and the tuple $(\mathbf{x}_0,\,\mathbf{x}_d)$ is added to the buffer.
...
To ensure meaningful training, it is important that the tuples in the training data always contain the \emph{intended} target position $x_d$, and not the target that was actually provided to the controller, i.e.,~$x_d + \Delta$.
...
\begin{algorithm}
    ...
    \For{each timestep $t_i$}{
        ...
        Simulate the system: $\mathbf{x} \gets response(\mathbf{x}_0, u)$\;
        Update system parameters\;
        Append $(\mathbf{x}_0, \mathbf{x}_d)$ to \textit{buffer}\;
...
\subsection{Preliminary Conclusions}\label{subsec:prelminary-conclusion}
These qualitative results show that the adapter is able to improve controller's behavior in a simple \emph{simulated} dynamic system.
The adapter is also making target adaptations that are intuitive and logical, given the specific settings of the system and controller.
The simulations thus provide a good foundation that the method is at least reasonable.
For a real-world test of the method, a demonstrator robot is required (\Cref{sec:origami-robot}) and a good test setup with a comprehensive set of metrics to quantify the results (\Cref{sec:experimental-setup}).
